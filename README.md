...
> 🌐 **[라이브 데모](https://hakhyun-kim.github.io/giwa-village/)는 기본 샘플입니다** —
> 서버 없이 도는 정적 데모(NPC 로컬 시뮬, 온체인은 실동작)라 마을 분위기만 보여줍니다.
> **실제 콘텐츠는 저장소를 클론해 `showcase.cmd`(자동 시연)로 확인해 주세요** —
> 키 조작 없이 노점 개설 → 온체인 구매 → 에스크로 정산 → 길드 던전까지
> 자막과 함께 자동 진행됩니다. ([실행 방법](#자동-시연-처음-보는-분))
