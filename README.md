...
- **GiwaMarketV3** [`0xfb28…dba7`](https://sepolia-explorer.giwa.io/address/0xfb28e3bc250e71c0015b95ea7ab5f919b83edba7)
  — **노점 레지스트리 온체인**: `openStall` 단일 tx로 개설+가격 기록, `buyStall`이
  가격을 체인에서 강제. + **에스크로**(확정 또는 24시간 후 정산 + **분쟁 처리**:
  구매자 `dispute`로 7일 연장, 판매자 `refund`로 즉시 환불)와 **ERC-1155 쿠폰**.
  `node scripts/deploy-village.mjs`로 재배포. (v2: `0x67cddd…ad67`, v1: `0x61491f…1357`)
- **GiwaHonors** [`0xdc1c…c648`](https://sepolia-explorer.giwa.io/address/0xdc1c3388129b4e9a5ab23a8bc3b90a5c7eafc648)
  — **소울바운드 칭호** (전송 함수가 아예 없는 온체인 기록 — 규제 안전선).
  개점·길드 창설자·등반가(10층)·고층 정복자(30층)를 다른 컨트랙트의 온체인 상태로
  검증해 클레임하고, 장착하면 모든 플레이어에게 이름표 배지로 보인다.
