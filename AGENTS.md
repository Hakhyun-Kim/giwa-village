...
| `window.__giwa` | `state()` `pos()` `teleport(x,z)` `ready()` `bossHit(n)` — 타격 연출만 흉내(가스 0) · `walls()` `blocked(x,z)` — 충돌 검사 · `mood(트랙, 남은체력)` — 도깨비를 실제로 깎지 않고 배경음 트랙만 갈아 끼운다 · `remotes()` — 주민·타 플레이어 좌표(걸음이 튀는지는 시간에 따라 찍어야 보인다) · `combat()` — 산채 원정의 구역 · 시각 · 단계(서버 없는 혼자 연습이 도는지) |
